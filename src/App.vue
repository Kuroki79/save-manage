...
async function scanUserFolder() {
  ...
  const result = await window.electronAPI.getUserFolderSaveList();
  if (result.success) {
    console.log(result);
    ...
    console.error('Failed to scan user folder:', result.message);
    scanCardLoading.value = false;
  }
}
...
async function importSteamGame() {
  ...
  if (checkedGames) {
    for (const element of checkedGames) {
        ...
        <v-container class="custom-container">
          <div class="container-wrapper">
            ...
            <v-dialog v-model="scanUserDialog" min-width="600px" width="auto" scrollable>
              <template v-slot:default>
                <v-card :loading="scanCardLoading" prepend-icon="mdi-application-import" title="扫描常用存档路径">
                  <template v-slot:loader="{ isActive }">
                    <v-progress-linear :active="isActive" color="deep-purple" height="4"
                      indeterminate></v-progress-linear>
                  </template>
